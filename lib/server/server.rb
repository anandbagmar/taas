...
post "/contract" do
  ...
  error_status = "\"failed\":\"#{output_hash.empty?}\""
  console_log = "\"console_log\":\"<Taas command output start>#{command_output}<Taas command output ends>\""

  "{#{console_log},#{error_status},#{data}}"
end
